...
class Server extends EventEmitter {
    ...
    switch (data.event) {
      ...
      case 'updated':
        if (!self.messageIsValid(data)) {
          return self.log.error('Invalid build message received', data);
        }

      case 'ready':
        var message = `Enforcing limits for ${data.build.id}`;
        if (data.build.project && data.build.project.organization) {
          message += ` for organization ${data.build.project.organization.id}`;
        }
  ...
  reap(build, done) {
    if (!build.container) {
      this.log.error(this.getLogContext(build), `Build ${build.id} lacks container information, skipping`);
      return done();
    }
    ...
    var urlParameters = querystring.stringify({
      force: true,
    ...
    request.del(`${self.containerManagerUrl}/containers/${containerId}?${urlParameters}`, function(error, response, body) {
      if (error) {
        self.log.error(Object.assign({}, self.getLogContext(build), querystring.parse(urlParameters)), `Container manager could not reap build`);
        return done(error);
      }

      self.log.info(Object.assign({}, self.getLogContext(build), querystring.parse(urlParameters)), `Reaping build`);

      // We consider a 404 successful because it
      // could mean this record has already been deleted.
      if (response.statusCode !== 200 && response.statusCode !== 404) {
        self.log.error(Object.assign({}, self.getLogContext(build), querystring.parse(urlParameters)), `Failed to reap build`);
        return done(new Error(body));
      }

      if (response.statusCode === 404) {
        self.log.warn(Object.assign({}, self.getLogContext(build), querystring.parse(urlParameters)), `A 404 was encountered while trying to reap container. Assuming the build has already been reaped and removing from the database.`);
      }
