...
class Server extends EventEmitter {
  ...
  messageProcessor(data, done) {
    var self = this;
    self.log.trace('Eventstream event received', data);
    if (data.event === 'ready') {
      if (!self.validateMessage(data)) {
        self.log.info(`Enforcing limits for ${data.build.id} for organization ${data.build.project.organization.id}`);
  ...
  getSubscriptionRule(build, rule) {
    ...
    return projectSpecificSetting !== false ? projectSpecificSetting : this.defaultLimits[rule];
  }

  reap(build, done) {
    this.log.info(`Reaping ${build.id} and removing container ${build.container.id}: http://localhost:3020/containers/${build.container.id}`);
    var self = this;
    var containerId = build.container.id;
    request.del(`http://localhost:3020/containers/${containerId}?force=true`, function(error, response, body) {
      self.log.info({containerId, buildId: build.id, event: 'reap'}, `Reaping ${build.id} received status code ${response.statusCode}`);
  ...
  enforceSizeLimit(build, done) {
    ...
    self.getOrganizationBuilds(organizationId, function(error, organizationBuilds) {
      ...
      self.log.info(`Organization ${organizationId} is using ${self.bytesToGigabytes(currentUsage)}G out of their ${self.bytesToGigabytes(limit)}G limit`, {buildId: build.id});
  ...
  aggregateSize(builds, done) {
    var reducer = function(size, build) {
      if (build.diskSpace.real !== -1) {
        return build.diskSpace.real + size;
      }
      return size;
    };
