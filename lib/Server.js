...
class Server extends EventEmitter {
  ...
  reap(build, done) {
    ...
    request.del(`${self.containerManagerUrl}/containers/${containerId}?force=true&reason=${build.reapedReason}`, function(error, response, body) {
      ...
      // We consider a 404 successful because it could mean this record has already been deleted.
      ...
      if (response.statusCode === 404) {
        self.log.warn(`A 404 was encountered while trying to reap container ${build.container.id} for build ${build.id}. Assuming the build has already been reaped and removing from the database.`);
      }
