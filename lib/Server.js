...
class Server extends EventEmitter {
  ...
  reap(build, done) {
    this.log.info(`Reaping ${build.id} and removing container ${build.container.id}`);
    var self = this;
    ...
    request.del(`${self.containerManagerUrl}/containers/${containerId}?force=true`, function(error, response, body) {
      ...
      if (response.statusCode !== 200) {
        this.log.error(`Failed to reap ${build.id} container ${build.container.id} with a DELETE to ${self.containerManagerUrl}/containers/${containerId}?force=true`);
