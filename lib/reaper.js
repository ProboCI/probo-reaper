...
function* removeContainer(containerId){
  var response
  try {
    response = yield requestAsync.del(`http://${probo_config.hostname}:${probo_config.port}/containers/${containerId}`)
  } catch(e){
    response = e.error
  }
...
function* start(){
  ...
  for(let project of projects){
    ...
    // TODO: delete all containers that are over the limit
    // for build in project.pull_requests.builds.slice(2)
    //   yield* removeContainer(build.container.id)
  }
}
