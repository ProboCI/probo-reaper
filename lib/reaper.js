...
function* buildsToProjects(builds) {
  ...
  var requester = Promise.promisify(function(project, pr, done) {
  ...
  function* getPRStatus(project, pr) {
    // TODO: This obviously needs to be managed in a generic way.
    return yield requester(project, pr);
  }
  ...
  for (let projectName in projects) {
    ...
    for (let pr in prs) {
      ...
      if (project.provider.slug === 'bitbucket') {
        pullRequest.state = yield* getPRStatus(project, pr);
      }
