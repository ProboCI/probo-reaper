...
function applyCriteria(project, criteria) {
  ...
  function stateFilter(state) {
    if (prStates.indexOf(state) < 0) {
      throw new TypeError(`state must be one of ${prStates}, found: ${state}`);
    }

    // If a PR does not have a state it is assumed to be 'open'
    return function(pr) {
      var myState = (pr.state && pr.state.state) || 'open';
  ...
  if (criteria.pullRequest) {
    for (let state of prStates) {
      ...
      if (stateCriteria && typeof stateCriteria.max != 'undefined') {
        ...
        for (let pr of project.pullRequests.filter(stateFilter(state))) {
  ...
  // TODO: ensure that if something is in 'keep', we remove it from 'remove'
