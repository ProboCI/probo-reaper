...
class Transform {
   ...
   */
  async buildsToProjects(arrayOfBuilds) {
    const builds = arrayOfBuilds[0];
    let projects = {};
    for (let build of builds) {
      const pullRequest =
        typeof build.pullRequest == 'object' ? build.pullRequest.number : null;
      if (pullRequest != null) {
        ...
        const branch =
          typeof build.branch == 'object' ? build.branch.name : build.branch + '';
      ...
      for (let project in projects) {
        if (projects.hasOwnProperty(project)) {
          ...
          projects[project].pullRequests = await this.processPullRequests(
            projects[project],
            pullRequests
          );
        }
  ...
  async processPullRequests(project, pullRequests) {
    ...
    for (let prName in pullRequests) {
      if (pullRequests.hasOwnProperty(prName)) {
        ...
        pullRequest.state = await this.provider.getPullRequest(
          project,
          pullRequest.id
        );
