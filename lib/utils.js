...
utils.Printer = class Printer {
  ...
  printBuilds(builds, indent) {
    ...
    indent = indent || '';
    for (let build of builds) {
      console.log(`${indent}Build ${build.id} ${build.createdAt} pr:${build.pullRequest} branch:${build.branch} container:${build.container.state} size: ${bytes(build.container.disk.containerSize)}`);
    }
  }
